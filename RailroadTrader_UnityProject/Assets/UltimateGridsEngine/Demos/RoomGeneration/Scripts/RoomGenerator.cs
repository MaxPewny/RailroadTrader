	...
	public void GenerateMap() {
		...
		for (int x = 0; x < m_MapSettings.m_MapSize.x; x ++) {
			for (int y = 0; y < m_MapSettings.m_MapSize.y; y ++) {
				...
				GridManager.Instance.InstantiateGridTile(m_TilePrefab, gridPosition, 0, mapHolder);
			}
		}
		...
		for (int i =0; i < obstacleCount; i ++) {
			...
			if (randomVector2Int != m_MapSettings.MapCentre && MapIsFullyAccessible(obstacleMap, currentObstacleCount)) {
				GridManager.Instance.InstantiateGridObject(m_ObstaclePrefab, randomVector2Int,  mapHolder);
		...
		GridManager.Instance.InstantiateGridObject(m_CharacterPrefab, m_MapSettings.MapCentre,  mapHolder);
	}
