...
public class GridObject : MonoBehaviour {
    ...
    [ReadOnly] public Vector2Int m_GridPosition = new Vector2Int(int.MaxValue, int.MaxValue);   // Default grid position on a non set GridObject
    [Header("Grid Tile")]
    [ReadOnly] public GridTile m_CurrentGridTile;  // Reference to the current GridTile this GridObject is at
    [ReadOnly] public GridTile m_PreviousGridTile; // Reference to the previous Gridtile we were at
    [Header("Facing Direction")]
    public Vector2Int m_InitialFacingDirection = new Vector2Int(0, 1);

    public Vector2Int m_FacingDirection {
        get {
            if (_facingDirection == Vector2Int.zero && m_InitialFacingDirection == Vector2Int.zero) {
    ...
    public virtual void Initialize(Vector3Int? gridPosition = null) {
        ...
        if (GridManager.ExistsTileAtPosition(m_GridPosition)) {
            GridTile gridTile = GridManager.GetGridTileAtPosition(m_GridPosition);
    ...
    public virtual bool BlocksMovement() {
        for (int i = 0; i < m_MovementBlockers.Count; i++) {
            if (m_MovementBlockers[i].BlocksMovement) {
                return true;
            }
        }
        return false;
